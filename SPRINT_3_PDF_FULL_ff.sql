...
# Creamos la Estructura de la tabla para las Tarjeta de Crédito
CREATE TABLE `transactions`.`credit_card` (
  ...
  # Inroducimos VALUES descargados INTO la tabla credit:card
...
# Comprobamos que estén todas las 275  filas

SELECT id, count(id) FROM credit_card;
# Miramos si hay algun id repetido
SELECT id, count(id) as contador FROM credit_card
group by id
having contador >1 
...
# sustiutir 'TR301950312213576817638661' por ' R323456312213576817699999 ' en campos indicados
...
# SPRINT 3 -  NIVEL 3  - EJERCICIO 1 
# a) Primer paso Creación de la nueva table User  usando los  dos ficheros proporcionados por el enunciado.
# b) Primero creamos el índice
CREATE INDEX idx_user_id ON transaction(user_id);
# c) Creamos la estructura de la base de datos
...
# d) Miramos que esté correctamente creada
describe user;
# e) Introducimos la data en la tabla, desactivamos y activamos de forma temporal el check de la llave foránea
...
# miramos que estén el total de 275 registros
select count(*) from user;

# f)Eliminamos la columna Website de la tabla company
alter table company drop column website;

# g)Modificamos el nombre del campo correo electrónico en la tabla user
ALTER TABLE user
RENAME COLUMN email TO personal_email;
describe credit_card;

# h)Modificamos tipo de datos, longitudes y datos nulos en la tabla credit_card
ALTER TABLE credit_card
...
# i)Añadimos la columna fecha_actual a la tabla credit_card
ALTER TABLE credit_card
ADD COLUMN `fecha_actual` DATE DEFAULT(current_date);
describe transaction;

# j)Añadimos las relaciones 1 a N en la tabla credit card
ALTER TABLE credit_card
...
# Constuir una vista llamada Informe Técnico que contenga datos de las tables user y credit_card con ciertos campos

CREATE VIEW view_informe_tecnico AS
			SELECT t.id, u.name, u.surname, d.iban, c.company_name
			FROM transaction as t
			JOIN company as c on c.id = t.company_id
			JOIN credit_card as d on d.id = t.credit_card_id
			JOIN user as u on u.id = t.user_id
            ORDER BY t.id desc;
            
SELECT *  from view_informe_tecnico
